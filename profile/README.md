...
- 구현 목록
    ...
    2. 장바구니 담기, 조회
    3. 현 재고 보다 많이 담을 경우 예외처리
    4. 장바구니 페이지 내에서 수량 조절 및 삭제
    5. 비회원은 쿠키를 사용하여 Redis에서 조회, 장바구니 정보는 30일간 유효
    6. 회원은 회원아이디를 사용하여 Redis에서 조회, 영구 유지
    7. 회원은 로그아웃 혹은 토큰이 만료되면 RDB에 Redis 정보를 복제하여 데이터 유지
...
### 인증/인가 
- 구현 목록
  1. front server에서 페이지 접근 마다 Interceptor를 통해 token 검증
